import { ingestDocument } from "../modules/audit/audit.service";
import { Doc } from "../modules/audit/types";

// ---------------------------------------------------------------------------
// Demo document set — a single source of truth shared by the CLI `npm run seed`
// script and the one-click "Load demo data" button in the web app.
//
// It is deliberately engineered to exercise every detector:
...
const REMOTE_ACCESS_V1 = `Remote Access Security Policy (v1) — Section 3: Endpoint Controls
...
const REMOTE_ACCESS_V2 = `Remote Access Security Policy (v2) — Section 3: Endpoint Controls
...
const REMOTE_ACCESS_V3 = `Remote Access Security Policy (v3) — Section 3: Endpoint Controls
...
const BYOD_POLICY = `Bring-Your-Own-Device (BYOD) Policy — Section 2: Personal Device Standards
...
const NOISE_TRAVEL = `Corporate Travel & Expense Policy — Section 1: Booking
...
interface DemoDocSpec {
  ...
  accessTier: string;
  content: string;
  previousKey?: string; // links this version to its predecessor for cross-version comparison
}

export const DEMO_DOC_SPECS: DemoDocSpec[] = [
  {
    key: "remoteV1",
    title: "Remote Access Security Policy (v1)",
    docType: "policy",
    version: "v1",
    accessTier: "tier-1",
    content: REMOTE_ACCESS_V1,
  },
  {
    key: "remoteV2",
    title: "Remote Access Security Policy (v2)",
    docType: "policy",
    version: "v2",
    accessTier: "tier-1",
    content: REMOTE_ACCESS_V2,
    previousKey: "remoteV1",
  },
  {
    key: "remoteV3",
    title: "Remote Access Security Policy (v3)",
    docType: "policy",
    version: "v3",
    accessTier: "tier-1",
    content: REMOTE_ACCESS_V3,
    previousKey: "remoteV2",
  },
  {
    key: "byod",
    title: "BYOD Policy",
  ...
  },
  {
    ...
    version: "v1",
    accessTier: "public",
    content: NOISE_TRAVEL,
  },
];

/**
 * Ingests the demo document set in dependency order, resolving previousKey
 * links to real document ids. Returns the ingested docs keyed by their spec key.
 */
export function seedDemoDocuments(): { docs: Doc[]; byKey: Record<string, Doc> } {
  ...
  for (const spec of DEMO_DOC_SPECS) {
    const doc = ingestDocument({
      ...
      previousDocumentId: spec.previousKey ? byKey[spec.previousKey].id : null,
    });
    byKey[spec.key] = doc;
    docs.push(doc);
  }
